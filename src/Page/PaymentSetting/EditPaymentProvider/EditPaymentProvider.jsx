...
const EditPaymentProvider = () => {
  ...
  const { data: providerDetail } = CustomGetFunction(
    `api/dashboard/payment-providers/${id}`,
    [id]
  );
  useEffect(() => {
  ...
  const EditHandler = async () => {
    try {
      ...
      const response = await axios.request(
        PostMethod(`api/dashboard/payment-providers/${id}`, fd)
      );

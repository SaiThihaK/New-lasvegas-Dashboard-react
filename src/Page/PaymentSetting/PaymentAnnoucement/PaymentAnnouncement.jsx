...
const PaymentAnnouncement = () => {
  ...
  const { data, loading } = CustomGetFunction(
    'api/admin/configuration-setting',
    [render]
  );
  console.log(data);
  useEffect(() => {
  ...
  const changeAnnouncement = async () => {
    try {
      const response = await axios.request(
        PostMethod('api/admin/change-configuration-setting', {
    ...
    <>
      {loading ? (
        <>
          ...
          <DepositeAnnouncement
            data={data}
            text={depositeText}
            setText={setDepositeText}
            render={render}
            setRender={setRender}
          />
        </>

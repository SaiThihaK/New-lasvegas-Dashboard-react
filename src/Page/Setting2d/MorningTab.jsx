...
const MorningTab = () => {
  ...
  //    }, [data])
  // const [adminOdd,setAdminOdd] = useState('');
  ...
  const twoDonoff = async (status) => {
    try {
      const response = await axios.request(
        PostMethod(
          `api/batchs/${setting.two_d.morning_batch.id}`,
          onOffSetting(status)
        )
      );
  ...
  const confirmHandler = async (type) => {
    try {
      const response = await axios.request(
        PostMethod(`api/2d-time-setting`, differenetBody(type))
      );
      if (response.data.status === 'success') {
        toast.success(response.data.message);
        return;
  ...
  return (
    <div>
      <Stack style={stackStyle} spacing={2} direction='row'>
        ...
        <Button
          variant='contained'
          onClick={() => confirmHandler('start_time')}
        >
          Save
        </Button>
      </Stack>
      <Stack style={stackStyle} spacing={2} direction='row'>
        ...
        <Button variant='contained' onClick={() => confirmHandler('end_time')}>
          Save
        </Button>
      </Stack>
      <Stack style={stackStyle} direction='row' spacing={2}>
        ...
        <p>
          တစ်နေ့တစ်ကြိမ်ပြင်ဆင်ပေးရန်။မပြင်ပါကထိုးသားထိုးလို့ရတော့မည်မဟုတ်ပါ။
        </p>
      </Stack>

      <Stack style={stackStyle} direction='row' spacing={2}>
        <label style={{ width: 200, marginTop: 10 }}>မနက်ပိုင်း</label>
      ...
      <Stack style={stackStyle} direction='row' spacing={2}>
        ...
        />
        <Button variant='contained' onClick={createAvaliable_amount}>
          Save
        </Button>
      </Stack>
      {/* <Stack style={stackStyle} direction="row" spacing={2}>

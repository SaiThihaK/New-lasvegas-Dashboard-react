...
const WithdrawModal = ({ ModalHandler, datas, tags }) => {
  ...
  const paymentSubmitHandler = async (event) => {
    ...
    try {
      const data = await axios.post(
        `${REACT_APP_DOMAIN}api/vegas-withdraw`,
        payment,
        { headers: { authorization: ltoken, Accept: 'application/json' } }
      );
      console.log(data);
  ...
  return (
    <div>
      <ToastContainer />
      <div
        class='bank-backdrop modal-none'
        onClick={() => {
          ModalHandler(false);
        }}
      ></div>
      <div className='k-card' style={{ width: '380px' }}>
        <div className='k-card-header'>
          <img src={datas?.logo} alt='' className='kpayImg' />
        </div>
          ...
          <div className='k-card-d'>
            ...
            <div>
              ...
              {validationError.hasOwnProperty('amount') && (
                <small className={'error'}>{validationError.amount[0]}</small>
              )}
            </div>
          </div>
          <div className='k-card-d'>
            ...
            <div>
              <input
                type='text'
                placeholder={`${datas.name} Number `}
                ref={accountNumber}
                className='kInp'
              />
              {validationError.hasOwnProperty('account_no') && (
                <small className={'error'}>
                  {validationError.account_no[0]}
                </small>
              )}
            </div>
          </div>
          <div className='k-card-d'>
            ...
            <div>
              <input
                ...
                className='kInp'
              />
              {validationError.hasOwnProperty('account_name') && (
                <small className={'error'}>
        ...
        <p className="k-long-para">*Enter this code in the Comments field when you make a deposit. If you do not
          ...
          <button
            ...
            className='k-confirm-button'
            style={{ marginBottom: '20px' }}
          >
            {' '}
            {submitLoading ? <Loading width={30} color='white' /> : 'Comfirm'}
          </button>
        </form>
      </div>
    </div>

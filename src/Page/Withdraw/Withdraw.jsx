...
const UserWithDrawTable = ({ }) => {
  ...
  const { data, loading, pagination } = CustomGetFunction(`api/user-withdraw?sortDirection=desc&receiver_user_id=${userData.id}&page=${page}`, [page, num]);
  console.log(data);
  ...
  const submitHandler = async (id) => {
    ...
    if (agent_action === 0) {
      try {
        const response = await axios.request(PostMethod(`api/agent/user-withdraw/action/rejected/${id}`, { remark }));
  ...
  const confirmHandler = async (id) => {
    ...
    if (agent_action === 1) {
      try {
        const response = await axios.request(PostMethod(`api/agent/user-withdraw/action/approve/${id}`));
  ...
  const differStatus = (user) => {
      ...
      return <StyledTableCell align="left">
        {
          id !== user.id ?
            (
              <Button sx={{ fontSize: "0.7rem" }} onClick={() => setId(user.id)} style={{ width: 120 }} color="success" variant="contained">
                Pending
              </Button>
            )
            : (<FormControl className={classes["form-control"]}>
              ...
              <Button onClick={() => { confirmHandler(user.id) }}>Confirm</Button>
            </FormControl>)
        }
    ...
    if (user.status !== ("pending" && "receiver-rejected")) {
      return <StyledTableCell align="left">
        <p className={classes["btn"]} style={{ backgroundColor: "blue", fontSize: "0.7rem" }} >Approve</p>
      </StyledTableCell>
    }
    if (user.status === "receiver-rejected") {
      return <StyledTableCell align="left">
        <p className={classes["btn"]} style={{ backgroundColor: "red", fontSize: "0.7rem" }} >Reject</p>
      </StyledTableCell>
    }
  }
 const columns = ['No','user-ID','User Name','Payment Provider','Amount','Phone/Account','Payment Date','User Action','Action']
  return (
    <PageCard title="ငွေထုတ်">
      ...
      {
        data.length !==0 &&  <CustomPagination setPage={setPage} totalPage={pagination?.meta?.last_page} />
      }

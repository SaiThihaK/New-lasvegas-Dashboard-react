...
import classes from './CreatePaymentAccountModal.module.css';
...
export default function CreatePaymentAccountModal({
  handleClose,
  open,
  setRender,
  render,
}) {
  const [setPayment_type] = useState([]);
  ...
  const Payment_ProviderChange = (e) =>
    setPayment_providerValue(e.target.value);
  ...
  const FetchPayment_provider = async () => {
    try {
      const response = await axios.request(
        getMethod(`api/dashboard/payment-providers`)
      );
      if (response.data.status === 'success') {
        // console.log(response.data.data);
  ...
  const createHandler = async () => {
    try {
      ...
      const response = await axios.request(
        PostMethod('api/dashboard/payment-accounts', fd)
      );
  ...
  const FilterPayment_Provider = payment_provider.filter(
    (c) => c.payment_type_id === payment_typeValue
  );
          ...
          <Box sx={style}>
            ...
            >
              ...
              <Select
                ...
                sx={{ backgroundColor: '#f3f3f3' }}
              >
                {FilterPayment_Provider &&
                  FilterPayment_Provider.map((item, index) => (
                    <MenuItem
                      key={index}
                      value={item.id}
                      style={{ display: 'flex' }}
                    >
                      <Avatar
                        src={FilterPayment_Provider.logo}
                        size='sm'
                        alt='logo'
                        variant='square'
                      />
            ...
            {type === 'agent' && (
              ...
              >
                ...
                <textarea
                  rows='4'
                  className={classes['remark_description']}
                  value={remark}
                  onChange={(e) => {
